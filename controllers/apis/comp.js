...
let apis = {
	 ...
	 * 数据验证用的API接口
	 */
	'validQuery': function (req, res, next) {
		let params = req.query;

		lib.connectMySQL().then(function (db) {
			// console.info('Got data from MySQL.');
			return EP.getValidPoints(db, params);
		}, function (err) {
			console.error('error: ', err);
		}).catch(function (error) {
			console.error('error: ', err);
	...
	'getJsonSum': function (req, res, next) {
		...
		let sqlstr = "SELECT ? AS 'name', MAX(wpnumber) AS 'wpnumber', MAX(vpnumber) AS 'vpnumber', MAX(wrnumber) AS 'wrnumber', MAX(vrnumber) AS 'vrnumber', MAX(prsval) AS 'prsval', MAX(trsval) AS 'trsval', MAX(arsval) AS 'arsval', MAX(ppsval) AS 'ppsval', MAX(tpsval) AS 'tpsval', MAX(apsval) AS 'apsval' FROM ?? WHERE 1;",
			sql = "";
