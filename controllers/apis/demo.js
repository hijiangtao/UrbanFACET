...
let demo = {
    ...
    tsnetrain(req, res, next) {
        ...
        shell.exec(`cd ${scriptdir} && python ./DecomposeFeature.py -qr 5 -tp ${}`).stdout
    ...
    entropyfilter(req, res, next) {
        ...
        shell.exec(`cd ${scriptdir} && python ./DecomposeFeature.py -qr 5 -et ${} -in ${} -ax ${} -tp ${}`).stdout
        if (lib.checkDirectory(path.join(scatterdir, oupscatterfile)) && lib.checkDirectory(path.join(imgdir, oupimgfile))) {
            recomdCal(scatterdir + '/tmp', oupscatterfile, id, res)
        } else {
            res.json({ 'scode': 0 })
        }
