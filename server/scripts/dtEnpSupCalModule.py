...
from entropy.grids import getGridsFromMongo, getPeopleDTEnpFromFile, filterByDaytype

class dtEnpSupCalModule(object):
	"""docstring for dtEnpSupCalModule"""
	def __init__(self, PROP, DATA):
		super(dtEnpSupCalModule, self).__init__()
	...
	def updateRecordEntropy(self, inputfile):
		...
		with open(inputfile, 'rb') as stream:
			for line in stream:
				...
				if filterByDaytype(devday, self.TIMEPERIOD):
					continue
...
def processTask(PROP, DATA):
	...
	task = dtEnpSupCalModule(PROP, DATA)
	task.run()
...
def main(argv):
	...
	# 处理输入参数
	city, directory, number, timeperiod, mode = 'zhangjiakou', '/home/tao.jiang/datasets/JingJinJi', 999, '7', 'all'
	for opt, arg in opts:
		if opt == '-h':
			usage()
			sys.exit()
...
if __name__ == '__main__':
	logging.basicConfig(filename='logger-dtenpsupcalmodule.log', level=logging.DEBUG)
	main(sys.argv[1:])
